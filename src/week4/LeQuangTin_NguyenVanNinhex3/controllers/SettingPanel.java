package week4.LeQuangTin_NguyenVanNinhex3.controllers;

import week4.LeQuangTin_NguyenVanNinhex3.entity.BracketValidator;
...
//        this.setResizable(false);
    ...
    private void declare() {
        ...
        sqrt = createCustomButton("√", Color.decode("#F2F6F7"), sizeKeyboard);
    }
    ...
    private void addActionListener() {
        ...
        sqrt.addActionListener(e -> {
            ...
            if (text.isEmpty() || !".^".contains(String.valueOf(text.charAt(text.length() - 1))))
                screen.setText(text + sqrt.getText());
            System.out.println(sqrt.getText());
        });
        ...
        equalSign.addActionListener(e -> {
            ...
            if (!text.isEmpty() && isNumeric(text.charAt(text.length() - 1) + "") ||
                ...
                if (text.matches(".*[-+x*÷/].*")) {
                    screen2.setText(
                            String.valueOf(bracket.inputTransformation(text + "="))
                    );

                } else {
                    if (text.contains("%"))
                        screen2.setText(
                                bracket.inputTransformation(text.replace("%", "") + "/100=")
                        );
                    else if (text.contains("^")) {
                        String[] split = text.split("\\^");
                        screen2.setText(
                                bracket.inputTransformation(split[0] + "^" + split[1] + "=")
                        );
                    } else
